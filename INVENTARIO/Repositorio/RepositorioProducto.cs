...
namespace INVENTARIO.Repositorio
{
    ...
    public class RepositorioProducto : IRepositorioProducto
    {
        ...
        public async Task<ProductoModel> ObtenerProductoPorId(int id)
        {
            ...
            {
                string query = @"SELECT Id, Codigo, Nombre, Marca, Descripcion, Precio, Unidades, Estado, urlimagen 
                                 FROM Producto 
                                 WHERE Id = @Id";
            ...
            try
            {
                using (var connection = new SqlConnection(cnx))
                {
                    var result = await connection.ExecuteAsync(
                        @"INSERT INTO Producto(Codigo, Nombre,Marca, Descripcion, Precio, Unidades, Estado, urlimagen) 
                          VALUES (@Codigo, @Nombre, @Marca, @Descripcion, @Precio, @Unidades, @Estado, @urlimagen)", producto
                    );
        ...
        public async Task<ProductoModel> Detalleproducto(int id)
        {
            using (IDbConnection db = new SqlConnection(cnx))
            {
                ...
                return await db.QuerySingleOrDefaultAsync<ProductoModel>(sqlQuery, new { Id = id });
            }
        }
